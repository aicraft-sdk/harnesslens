/**
 * Minimal scan-configuration types. See NOTICE for third-party attribution.
 *
 * Phase 1 only needs the *shape* of a resolved scan configuration so
 * `score.ts`/`types.ts`/`harness/global-paths.ts` compile against the
 * same contract. The `.harness-audit.json` file-driven config
