...
describe('GET /repos/:org/:repo and /history', () => {
  ...
  it('GET /repos/:org/:repo -> 200 latest submission for a public repo', async () => {
    await seedSubmission('acme/widgets', { scannedAt: '2026-08-01T00:00:00Z' });
    await seedSubmission('acme/widgets', { scannedAt: '2026-08-10T00:00:00Z' });

    const res = await request(app.getHttpServer()).get('/repos/acme/widgets').expect(200);

    expect(res.body.repoId).toBe('acme/widgets');
    expect(res.body.scannedAt).toBe('2026-08-10T00:00:00.000Z');
  });

  it('GET /repos/:org/:repo -> 404 for a private repo (unauthenticated)', async () => {
    await seedSubmission('acme/private-repo', {}, { visibility: 'private' });

    await request(app.getHttpServer()).get('/repos/acme/private-repo').expect(404);
  });

  it('GET /repos/:org/:repo -> 404 for a repoId that has never submitted', async () => {
    await request(app.getHttpServer()).get('/repos/acme/never-existed').expect(404);
  });

  it('GET /repos/:org/:repo -> 404 with an identical response shape for a private repo and a repo that never existed', async () => {
    await seedSubmission('acme/private-repo', {}, { visibility: 'private' });
  ...
  it('GET /repos/:org/:repo/history -> 200 array ordered newest-first, all entries for a public repo', async () => {
    await seedSubmission('acme/widgets', { scannedAt: '2026-08-01T00:00:00Z' });
    await seedSubmission('acme/widgets', { scannedAt: '2026-08-10T00:00:00Z' });
    await seedSubmission('acme/widgets', { scannedAt: '2026-08-05T00:00:00Z' });

    const res = await request(app.getHttpServer()).get('/repos/acme/widgets/history').expect(200);

    expect(res.body).toHaveLength(3);
    expect(res.body.map((entry: { scannedAt: string }) => entry.scannedAt)).toEqual([
  ...
  it('GET /repos/:org/:repo/history -> 404 for a private repo (unauthenticated)', async () => {
    await seedSubmission('acme/private-repo', {}, { visibility: 'private' });

    await request(app.getHttpServer()).get('/repos/acme/private-repo/history').expect(404);
  });
});
